      ...
      <div className="max-w-6xl mx-auto">
        <header className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-black text-slate-900 uppercase tracking-tight">Gestão de Bilhetes</h1>
              <p className="text-slate-500 text-sm">Monitoramento administrativo em tempo real.</p>
            </div>
          </div>
          <div className="flex gap-2">
            <Button onClick={() => navigate({ to: "/" as any })} variant="outline" className="border-slate-200">Home</Button>
            <Button onClick={loadTickets} variant="outline" className="border-emerald-200 text-emerald-600 hover:bg-emerald-50">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-4 md:p-6 mb-8 shadow-sm">
          <div className="relative max-w-md">
            ...
            <Input 
              ...
              className="pl-10 bg-slate-50 border-slate-200 text-slate-900 placeholder:text-slate-400 focus:ring-emerald-500"
            />
          </div>
        </div>
        ...
        {loading && tickets.length === 0 ? (
          ...
          <div className="text-center p-20 bg-white rounded-2xl border border-slate-200 shadow-sm">
            ...
            <h3 className="text-xl font-bold text-slate-900 uppercase">Nenhum bilhete encontrado</h3>
            ...
            {tickets.map(t => (
              <div 
                ...
                className={cn(
                  ...
                  expandedTicketId === t.id ? "ring-2 ring-emerald-500/20 border-emerald-500" : "hover:border-emerald-200"
                )}
              >
                <div 
                ...
                >
                  <div className="grid grid-cols-2 sm:grid-cols-4 lg:flex lg:items-center gap-6 flex-1">
                    <div className="space-y-1">
                      <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest block">Código</span>
                      <span className="text-lg font-black text-slate-900 tracking-widest">{t.code}</span>
                    </div>

                    <div className="space-y-1 min-w-[140px]">
                      <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest block">Usuário</span>
                    ...
                    <div className="space-y-1">
                      <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest block">Telefone</span>
                    ...
                    <div className="space-y-1">
                      <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest block">Data/Hora</span>
                      <div className="flex items-center gap-1.5 text-xs text-slate-500">
                        <Calendar size={12} className="text-emerald-600" />
                        {new Date(t.created_at).toLocaleDateString('pt-BR')}
                        <Clock size={12} className="ml-1 text-emerald-600" />
                  ...
                  <div className="flex items-center justify-between lg:justify-end gap-8 pt-4 lg:pt-0 border-t lg:border-t-0 border-slate-100">
                    <div className="flex flex-col text-right">
                      <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Aposta</span>
                    ...
                    <ChevronRight 
                      className={cn("text-slate-400 transition-transform duration-300 hidden lg:block", expandedTicketId === t.id && "rotate-90 text-emerald-600")} 
                      size={20} 
                    />
                  </div>
                </div>
                ...
                {expandedTicketId === t.id && (
                  <div className="border-t border-slate-100 bg-slate-50/50 p-5 md:p-8 animate-in slide-in-from-top-2 duration-300">
                    <div className="space-y-6">
                          ...
                          <div key={idx} className="bg-white border border-slate-200 rounded-2xl p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-sm">
                            <div className="space-y-2">
                              ...
                              <div className="flex items-center gap-3 text-sm text-slate-600 font-bold">
                                ...
                                <span className="text-emerald-600 px-1 font-black">vs</span> 
                                {sel.away_team_snapshot}
                              </div>
                              <div className="flex items-center gap-4 text-[11px] text-slate-400 font-medium">
                                <span className="flex items-center gap-1"><Calendar size={12} className="text-emerald-600" /> {sel.league_name_snapshot}</span>
                                <span className="flex items-center gap-1"><Clock size={12} className="text-emerald-600" /> {new Date(sel.kickoff_at_snapshot).toLocaleString('pt-BR')}</span>
                              </div>
                            </div>
                            <div className="flex items-center justify-between md:justify-end gap-10 pt-4 md:pt-0 border-t md:border-t-0 border-slate-100">
                              <div className="flex flex-col md:items-end">
                                <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Odd Salva</span>
                                <span className="text-xl font-black text-emerald-600">{sel.odd_snapshot.toFixed(2)}</span>
                              </div>
                              <div className="flex flex-col md:items-end">
                                <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Resultado</span>
                      ...
                      <div className="bg-white rounded-2xl p-5 border border-slate-200 flex flex-wrap items-center justify-between gap-6 shadow-sm">
                        <div className="flex items-center gap-8">
                          <div className="flex flex-col">
                            <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Odd Acumulada</span>
                            <span className="text-lg font-black text-slate-900">{t.total_odd.toFixed(2)}</span>
                          </div>
                          <div className="flex flex-col">
                            <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Valor do Bilhete</span>
                            <span className="text-lg font-black text-slate-900">R$ {t.stake.toFixed(2)}</span>
                          </div>
                        </div>
