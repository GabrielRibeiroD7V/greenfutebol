...
import { Ticket, AlertCircle, ArrowLeft, Trash2, CheckCircle2, Loader2 } from "lucide-react";
...
function BilhetePage() {
    const { selections, previewTotalOdd, stake, removeSelection, clearBetSlip, idempotencyKey, generateIdempotencyKey } = useBetSlip();
    ...
    if (ticketResult) {
        return (<div className="min-h-screen bg-[#050505] text-white flex flex-col items-center justify-center p-4 max-w-2xl mx-auto text-center space-y-6">
    ...
    return (<div className="min-h-screen bg-[#050505] text-white p-4 max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-zinc-900 border border-emerald-500/20 p-6 rounded-3xl space-y-6 shadow-[0_0_30px_rgba(16,185,129,0.05)]">
        ...
        <div className="space-y-3">
          <Button onClick={handleConfirm} disabled={isSubmitting} className="w-full h-14 bg-emerald-600 hover:bg-emerald-500 text-white font-black uppercase tracking-widest rounded-xl shadow-[0_0_30px_rgba(16,185,129,0.2)] disabled:opacity-50">
            {isSubmitting ? (<Loader2 className="animate-spin"/>) : ('Confirmar Bilhete')}
          </Button>
