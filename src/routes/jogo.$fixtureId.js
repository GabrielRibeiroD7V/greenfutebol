...
import { ArrowLeft, Clock, Loader2, Calendar, MapPin, Info } from "lucide-react";
    ...
    return (<div className="min-h-screen bg-[#050505] text-white flex flex-col lg:flex-row">
      <div className="flex-1 min-w-0">
        <header className="sticky top-0 z-40 bg-black/80 backdrop-blur-md border-b border-white/5 p-4 flex items-center justify-between">
          ...
          {isAdmin && (<button onClick={() => navigate({ to: `/admin/mercados/${fixture.fixture_id}` })} className="text-[10px] font-black uppercase text-zinc-500 hover:text-emerald-500 underline">
              Gerenciar mercados
            </button>)}
        </header>
        ...
        <main className="p-4 space-y-6 max-w-4xl mx-auto w-full">
          ...
          <div className="space-y-4 pb-24">
            {markets.length === 0 ? (<div className="bg-zinc-900/50 border border-white/5 rounded-3xl p-12 text-center space-y-4">
            ...
            .map(m => (<MarketRenderer key={m.id} market={m} fixture={{
