...
import { ArrowLeft, Clock, Loader2, Info, Calendar, MapPin } from "lucide-react";
...
function MatchDetails() {
    ...
    const [expandedCategories, setExpandedCategories] = useState({ "Principais": true });
    ...
    if (isLoading)
        return <div className="min-h-screen bg-black flex items-center justify-center"><Loader2 className="animate-spin text-emerald-500"/></div>;
    if (!fixture)
        return <div className="min-h-screen bg-black text-white p-8">Partida não encontrada</div>;
    const isStarted = new Date(fixture.kickoff_at) <= new Date();
    return (<div className="min-h-screen bg-[#050505] text-white flex flex-col lg:flex-row">
      <div className="flex-1 min-w-0">
        <header className="sticky top-0 z-40 bg-black/80 backdrop-blur-md border-b border-white/5 p-4 flex items-center justify-between">
          ...
          {isAdmin && (<button onClick={() => navigate({ to: `/admin/odds`, search: { fixture_id: fixture.fixture_id } })} className="text-[10px] font-black uppercase text-zinc-500 hover:text-emerald-500 underline">
              Gerenciar mercados
            </button>)}
        </header>
        ...
        <main className="p-4 space-y-6 max-w-4xl mx-auto w-full">
          ...
          <div className="space-y-4 pb-24">
            {markets.length === 0 ? (<div className="text-center py-12 opacity-50">Nenhum mercado disponível no momento.</div>) : (markets.map(m => (<div key={m.id} className="bg-zinc-900 border border-white/5 rounded-2xl overflow-hidden">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3">
                    {m.fixture_market_selections?.sort((a, b) => a.sort_order - b.sort_order).map((s) => {
                ...
                return (<button key={s.id} disabled={isDisabled || isPastKickoff} onClick={() => {
                        if (isPastKickoff) {
                    ...
                    }} className={cn("p-4 border-r border-b border-white/5 flex flex-col items-center justify-center transition-all", selected ? "bg-emerald-600" : "hover:bg-white/5", (isDisabled || isPastKickoff) && "opacity-30 cursor-not-allowed grayscale")}>
