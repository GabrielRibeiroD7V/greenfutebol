...
import { Trophy, Zap, ShieldCheck, Clock, ChevronRight } from "lucide-react";
      ...
      <main className="container mx-auto px-4 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            {isLoading ? (<div className="flex justify-center py-20"><Zap className="animate-spin text-emerald-500"/></div>) : fixtures.length === 0 ? (<div className="text-center py-20 text-zinc-600 font-bold uppercase tracking-widest">Nenhuma partida encontrada</div>) : (fixtures.map((f) => (<div key={f.fixture_id} className="group bg-zinc-900/50 border border-white/5 rounded-3xl p-6 hover:border-emerald-500/30 transition-all relative overflow-hidden">
                  ...
                  <div onClick={() => navigate({ to: `/jogo/${f.fixture_id}` })} className="flex items-center justify-around gap-4 mb-8 cursor-pointer">
                  ...
                  <div className="flex justify-center">
                    <Button onClick={() => navigate({ to: `/jogo/${f.fixture_id}` })} variant="outline" className="border-emerald-500/20 text-emerald-500 hover:bg-emerald-500 hover:text-black font-black uppercase tracking-widest text-xs h-10 px-8 rounded-xl">
