...
export function BetSlip() {
    const { selections, removeSelection, clearBetSlip, previewTotalOdd, stake, setStake } = useBetSlip();
    ...
    return (<div className="flex flex-col h-full bg-[#050505]">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {selections.length === 0 ? (<div className="h-full flex flex-col items-center justify-center text-center py-10 opacity-30">
          ...
          </div>) : (selections.map((s) => (<div key={s.selectionId} className="bg-zinc-900 border border-white/5 rounded-xl p-3 relative group animate-in slide-in-from-right-2">
              <button onClick={() => removeSelection(s.selectionId)} className="absolute top-2 right-2 text-zinc-600 hover:text-red-500 transition-colors">
                <X size={14}/>
              </button>
      ...
      {selections.length > 0 && (<div className="p-4 bg-zinc-950 border-t border-white/5 space-y-4">
          <div className="space-y-3">
            ...
            <div className="space-y-1">
              ...
              <input type="number" value={stake} onChange={(e) => setStake(Number(e.target.value))} className="w-full bg-zinc-900 border border-white/10 rounded-lg py-2 px-3 text-white font-black text-lg focus:ring-1 focus:ring-emerald-500 outline-none"/>
            </div>
          ...
          <div className="space-y-2">
            <Button onClick={handleContinue} className="w-full h-12 bg-emerald-600 hover:bg-emerald-500 text-white font-black uppercase tracking-widest rounded-xl shadow-[0_0_20px_rgba(16,185,129,0.2)]">
              Continuar
            </Button>
